...
async function renderStudents(){
  ...
  document.getElementById("students").innerHTML=`
    ...
    <div class="list">${invited.map(r=>`<div class="list-item"><div class="icon">📲</div><div style="flex:1"><strong>${r.name}</strong><div class="small muted">${r.phone||""} • aguardando o aluno completar</div></div>${professor?`<button class="mini-btn" onclick="shareRegistrationInvite('${r.id}')">Reenviar</button>`:""}</div>`).join("")}</div>`:""}
