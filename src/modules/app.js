...
async function renderCheckin(){
  ...
  document.getElementById("checkin").innerHTML=`<div class="section-title"><h2>Check-in do treino</h2><span class="pill green">${att.filter(a=>a.date===todayISO()&&a.status==="approved").length} presente(s)</span></div>${session?`<div class="card session-card"><div class="small muted">AULA ABERTA</div><h3>${session.title}</h3><div id="sessionQr" class="qr-host"></div><div class="session-code">${session.code}</div><p class="small muted">O aluno pode ler este QR com o próprio celular ou digitar o código.</p><div class="actions"><button class="btn danger" onclick="closeClassSession('${session.id}')">Encerrar</button><button class="btn secondary" onclick="copySessionCode('${session.code}')">Copiar código</button></div></div>`:`<details class="card"><summary style="cursor:pointer;font-weight:700">➕ Aula avulsa</summary><p class="small muted" style="margin-top:8px">Só para treino fora da grade (aberto, seminário, reposição). As turmas do dia se abrem em <strong>Abrir aula</strong>, lá em cima.</p><div class="field"><label>Nome da aula</label><input id="classTitle" value="Treino avulso ${new Date().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}"></div><div class="field"><label>Validade</label><select id="classDuration"><option value="60">1 hora</option><option value="90" selected>1h30</option><option value="120">2 horas</option></select></div><button class="btn primary full" onclick="openClassSession()">Abrir aula avulsa e gerar QR</button></details>`}
...
async function renderStudents(){
  ...
  document.getElementById("students").innerHTML=`
    <div class="section-title"><h2>Alunos</h2>${professor?`<div class="actions"><button class="btn secondary" onclick="shareSelfRegistrationLink()">📩 Aluno preenche</button><button class="btn primary" onclick="newStudentModal()">✍️ Eu cadastro</button></div>`:`<span class="pill">Somente leitura</span>`}</div>
...
function newStudentModal(){
  showModal(`<h3>✍️ Cadastrar aluno</h3>
    <p class="small muted">Com telefone, o aluno recebe o link e completa o cadastro sozinho. Sem telefone, a ficha é criada do mesmo jeito e você cuida de tudo por aqui.</p>
    ...
    <div class="field"><label>Telefone / WhatsApp <span class="small muted">(opcional)</span></label><input id="nsPhone" inputmode="tel" placeholder="(48) 99999-9999" oninput="updateQuickSaveLabel()"></div>
    <div class="field"><label>Trilha</label><select id="nsTrack"><option value="adulto">Adulto</option><option value="kids">Kids</option></select></div>
    <p class="small muted" id="nsNoPhoneHint">Sem telefone: a ficha é criada e o Professor marca presença pelo check-in manual. O aluno não usa o app.</p>
    <button class="btn primary full" id="nsSaveBtn" onclick="saveQuickStudentInvite()">Cadastrar ficha</button>
...
function updateQuickSaveLabel(){
  const tem=!!document.getElementById("nsPhone")?.value.trim();
  const b=document.getElementById("nsSaveBtn"),h=document.getElementById("nsNoPhoneHint");
  if(b)b.textContent=tem?"Cadastrar e enviar link":"Cadastrar ficha";
  if(h)h.style.display=tem?"none":"";
}
async function saveQuickStudentInvite(){
  ...
  const track=document.getElementById("nsTrack")?.value||"adulto";
  if(!phone){                                   // Kids sem contato, ou quem não usa celular
    const belt=track==="kids"?"Cinza":"Branca";
    const student={id:uid("s"),name,nickname:name.split(" ")[0],phone:"",birth:"",graduationTrack:track,belt,stripes:0,
      classesInBelt:0,points:0,payment:"liberado",dueDay:10,active:true,createdAt:new Date().toISOString()};
    await DB.put("students",student);
    await TonicaoAuth.audit("student.create","Aluno cadastrado sem contato",student.id,{graduationTrack:track});
    closeModal();toast(`${name.split(" ")[0]} cadastrado. Marque a presença pelo check-in manual.`);
  ...
  const settings=await getSettings();
  let req={id:uid("reg"),academyId:settings.academyId||"tonicao-sul-ilha",name,phone,graduationTrack:track,status:"invited",source:"professor_invite",inviteToken:localInviteToken(),createdAt:new Date().toISOString(),invitedBy:me.id};
  try{
...
async function saveNewStudent(){
  ...
  await DB.put("students",student);
  closeModal();await TonicaoAuth.audit("student.create","Aluno cadastrado",student.id,{graduationTrack,phone:student.phone});toast("Aluno cadastrado.");await renderAll();if(student.phone)setTimeout(()=>quickInvite(student.id),120)
}
...
// v0.43: quantidade separada do nome, com − e +.
function splitReq(text){
  const m=String(text||"").trim().match(/^(\d+)\s+(.*)$/);
  if(!m)return {qty:1,name:String(text||"").trim()};
  return {qty:Math.max(1,+m[1]||1),name:m[2].trim()};
}
function joinReq(qty,name){return `${Math.max(1,qty)} ${name}`.trim()}
...
async function showPlanChecklist(planId){
  ...
  const emCurso=(await DB.getAll("gradingAssignments")).filter(a=>a.planId===planId&&a.status!=="done").length;
  showModal(`<div class="exam-title">${esc(p.name)}</div>
    <p class="small muted">Ajuste a quantidade de cada item e vincule um vídeo da Biblioteca.</p>
    <div class="checklist">${p.practical.map((x,i)=>{
      const {qty,name}=splitReq(x);
      const linked=(links["p"+i]||[]).map(id=>techs.find(t=>t.id===id)).filter(Boolean);
      return `<div class="check-item"><div style="flex:1">
        <strong>${esc(name)}</strong>
        <div style="display:flex;align-items:center;gap:10px;margin:8px 0">
          <button class="mini-btn" onclick="changeReqQty('${planId}',${i},-1)" ${qty<=1?"disabled":""}>−</button>
          <span class="pill">${qty}×</span>
          <button class="mini-btn" onclick="changeReqQty('${planId}',${i},1)">+</button>
        </div>
        <div class="small muted">${linked.length?"Vinculado: "+esc(linked.map(t=>t.title).join(", ")):"Sem vídeo vinculado"}</div>
        <button class="mini-btn" onclick="linkTechniqueModal('${planId}',${i})">Vincular técnica</button>
      </div></div>`}).join("")}</div>
    <div class="notice small" style="margin-top:12px"><strong>⭐ Esta é a lista oficial da faixa ${esc(p.targetBelt)}</strong>
      <div class="small">O que você mudar aqui vale para todas as próximas graduações desta faixa${emCurso?`, e também para ${emCurso} exame(s) em andamento`:""}.</div></div>
    <button class="btn secondary full" style="margin-top:8px" onclick="closeModal()">Fechar</button>`)
}
window.changeReqQty=async function(planId,index,delta){
  if(!(await guard("manage_graduation")))return;
  const p=await DB.getOne("gradingPlans",planId);if(!p)return;
  const {qty,name}=splitReq(p.practical[index]);
  const novo=Math.max(1,Math.min(99,qty+delta));
  p.practical[index]=joinReq(novo,name);
  await DB.put("gradingPlans",p);
  showPlanChecklist(planId);
};
...
/* v0.43: o link de primeira configuração sai da tela de entrada.
...
// v0.43: só redesenha a tela quando a pessoa não está no meio de alguma coisa.
...
(()=>{
  ...
  window.bookingsModal=async function(scheduleId,date){
    ...
    const sessIds=(await getSessions()).filter(x=>x.date===date&&(x.scheduleId===scheduleId||!scheduleId)).map(x=>x.id);
    const att=(await getAttendance()).filter(a=>a.date===date&&(sessIds.includes(a.sessionId)||a.scheduleId===scheduleId)).sort((a,b)=>String(a.time).localeCompare(String(b.time)));
    const marked=new Set(att.map(a=>a.studentId));
    showModal(`<h3>Presenças • ${esc(sc?.name||"Turma")} ${esc(sc?.start||"")}</h3><p class="small muted">${esc(dayLabel(date))}</p>
      <div class="section-title"><h2>✅ Fizeram check-in (${att.length})</h2></div>
      <div class="list">${att.length?att.map(a=>`<div class="list-item" style="cursor:default"><div class="icon">${a.status==="approved"?"✅":"⏳"}</div><div class="meta"><strong>${esc(name(a.studentId))}</strong><span class="small muted">${esc(a.time||"")} • ${a.status==="approved"?"confirmado":"aguardando sincronizar"}${a.source==="professor"?" • manual":""}</span></div></div>`).join(""):`<div class="notice small">Ninguém fez check-in ainda. Abra a aula e mostre o QR.</div>`}</div>
      <div class="section-title" style="margin-top:12px"><h2>📅 Marcaram antes (${list.length})</h2></div>
      <div class="list">${list.length?list.sort((a,b)=>name(a.studentId).localeCompare(name(b.studentId))).map(b=>`<div class="list-item" style="cursor:default"><div class="meta"><strong>${esc(name(b.studentId))}${marked.has(b.studentId)?" ✅":""}</strong><span class="small muted">${STATUS[b.status]?.[0]||b.status}</span></div>${b.status==="pending"?`<div class="actions" style="flex-wrap:nowrap"><button class="btn green" onclick="decideBooking('${esc(b.id)}',true)">✅ Veio</button><button class="btn secondary" onclick="decideBooking('${esc(b.id)}',false)">❌</button></div>`:""}</div>`).join(""):`<div class="notice">Ninguém marcou presença antes pelo app.</div>`}</div>
  ...
  async function professorSection(){
    ...
    const bk=await bookings();const allSessions=await getSessions();const session=activeSession(allSessions);const att=await getAttendance();
    ...
    return `<div class="section-title"><h2>📅 Aulas de hoje</h2><button class="btn secondary" onclick="scheduleManagerModal()">⚙️ Grade</button></div>
      ...
      <div class="list">${sch.length?await Promise.all(sch.map(async s=>{const n=bk.filter(b=>b.scheduleId===s.id&&b.date===today).length;const open=session&&session.scheduleId===s.id;
        const ids=allSessions.filter(x=>x.date===today&&x.scheduleId===s.id).map(x=>x.id);
        const pres=att.filter(a=>a.date===today&&(ids.includes(a.sessionId)||a.scheduleId===s.id)).length;
        return `<div class="list-item" style="cursor:default"><div class="icon">🥋</div><div class="meta"><strong>${esc(s.start)}–${esc(endTime(s.start,s.duration))} • ${esc(s.name)}</strong><span class="small muted">✅ ${pres} presente(s)${n?` • ${n} marcaram antes`:""}</span></div>
          <div class="actions" style="flex-wrap:nowrap">${open?`<button class="btn green" onclick="showQrFullscreen()">📺 QR</button>`:`<button class="btn primary" onclick="openScheduledClass('${esc(s.id)}')">Abrir aula</button>`}<button class="btn secondary" onclick="bookingsModal('${esc(s.id)}','${today}')">📋</button></div></div>`})).then(x=>x.join(""))
